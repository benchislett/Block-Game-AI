...
uint64_t countTerminalNodes(const Board& board, int depth, int max_depth, const std::vector<Piece>& pieces) {
        ...
        for (int row = 0; row < piece.shiftTable.maxRow + 1; ++row) {
            for (int col = 0; col < piece.shiftTable.maxCol + 1; ++col) {
                ...
                if (board.canPlace(mask)) {
                    ...
                    total_terminal_nodes += countTerminalNodes(next_board, depth + 1, max_depth, pieces);
                }
            }
        }
    }
...
int main(int argc, char* argv[]) {
    ...
    for (int d = 0; d <= max_depth_limit; ++d) {
        ...
        uint64_t count = countTerminalNodes(initialBoard, 0, d, pieces);
